...
var MultiSelectSearchFilter = (function () {
    ...
    MultiSelectSearchFilter.prototype.transform = function (options, str, limit, renderLimit) {
        ...
        var matchPredicate = function (option) { return (str == '' || option.name.toLowerCase().indexOf(str.toLowerCase()) >= 0); }, getChildren = function (option) { return options.filter(function (child) { return child.parentId === option.id; }); }, getParent = function (option) { return options.find(function (parent) { return option.parentId === parent.id; }); };
        var i = 0, founded = 0;
        for (; i < optsLength && founded < maxFound; ++i) {
            var option = options[i];
			
            var directMatch = (str == '' || option.name.toLowerCase().indexOf(str.toLowerCase()) >= 0);
            if (directMatch) {
